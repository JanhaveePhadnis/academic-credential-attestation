...
const required = ['README.md', 'PROPOSAL.md', 'TESTING.md', 'src/test/degree.test.ts', 'deployment.json', 'SECURITY.md', 'OPERATIONS.md', 'src/runtimeConfig.ts', 'src/test/runtime-config.test.ts'];
...
const [readme, proposal, testing, suite, deploymentText] = await Promise.all([
  ...
  readFile('src/test/degree.test.ts', 'utf8'),
...
if (!testing.includes('src/test/degree.test.ts')) throw new Error('TESTING.md must identify the executable suite.');
...
if (deployment.contract !== 'degree' && deployment.contractName !== 'degree') {
  throw new Error('Deployment manifest does not identify the degree contract.');
}
...
console.log('Registrar evidence audit: proposal, src/test/degree.test.ts, and Preview deployment evidence verified.');
