# Operations runbook — Academic Credential Attestation
...
2. Confirm contract `degree` and its address against `deployment.json`.
3. Confirm the wallet has sufficient NIGHT/DUST and is synchronized.
4. Load every proving/verifier asset from `public/midnight/degree`.
...
Suspend attestations from the affected registrar, retain public transaction evidence, rotate trust data, and direct students to the issuing institution.
