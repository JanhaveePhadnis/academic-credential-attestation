...
const degreeWitnesses = {
  localSecretKey: ({ privateState }) => [privateState, privateState.secretKey],
  degreeSubject: ({ privateState }) => [privateState, privateState.degreeSubject],
  universitySignature: ({ privateState }) => [privateState, privateState.universitySignature],
};
...
async function deploy() {
  ...
  const zkConfigPath = path.resolve('contracts', 'managed', 'degree');
  const contractModule = await import(path.resolve(zkConfigPath, 'contract', 'index.js'));
  const compiledContract = CompiledContract.make('degree', contractModule.Contract).pipe(
    CompiledContract.withWitnesses(degreeWitnesses),
    CompiledContract.withCompiledFileAssets(zkConfigPath),
  );
  console.log('Degree contract loaded.');
  ...
  const initialPrivateState = {
    ...
    degreeSubject: new Uint8Array(32),
    universitySignature: new Uint8Array(32),
  };
  ...
  const providers = {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'degree-private-state',
      accountId,
  ...
  const bootstrapContract = new contractModule.Contract(degreeWitnesses);
  ...
  console.log('Generating ZK proofs & deploying Degree contract (takes 30-60 seconds)...');
  ...
  console.log('\n=== DEGREE CONTRACT SUCCESSFULLY DEPLOYED ===');
